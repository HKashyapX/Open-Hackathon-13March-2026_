    ...
    <div className="min-h-screen bg-[#0a0a0c] text-slate-200 font-sans p-4 md:p-10">
      ...
      <div className="max-w-6xl mx-auto flex justify-between items-center mb-10 border-b border-white/10 pb-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-black tracking-widest text-white">AEGIS<span className="text-blue-500">MESH</span></h1>
        </div>
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-xl">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h2 className="text-sm font-bold uppercase tracking-widest text-slate-400">Nodes in Range</h2>
          </div>
          <div className="space-y-3">
            {peers.length === 0 ? (
              <div className="py-10 text-center border border-dashed border-white/10 rounded-xl">
                <p className="text-xs text-slate-500">Broadcasting Discovery Signal...</p>
              </div>
            ) : (
              peers.map(p => (
                <div key={p} className="bg-white/5 p-4 rounded-xl border border-white/5 hover:border-blue-500/30 transition-all cursor-pointer group">
                  <p className="text-[10px] text-slate-500 mb-1">AUTHORIZED NODE</p>
                  <p className="text-sm font-mono text-blue-100 group-hover:text-blue-400 transition-colors">{p}</p>
                </div>
              ))
            )}
          </div>
